...
Vector3f Scene::castRay(const Ray& ray, int depth) const
{
	...
	Vector3f direct_lighting;
	Vector3f indirect_lighting;
	float pdf ;
	
	sampleLight(light_intersection, pdf);

	Vector3f N = intersection.normal.normalized();
	//ws
	Vector3f light_dir = (light_intersection.coords - intersection.coords).normalized();
	//NN
	...
	Vector3f shadow_orig = intersection.coords ;
	...
	Intersection testLightHit= intersect(shadow_ray);
	...
	{ 
		...
		Ray reflection_ray(intersection.coords , reflection_dir);


		Intersection reflection_intersection = intersect(reflection_ray);
		if (reflection_intersection.happened && (!reflection_intersection.m->hasEmission()))
		{


			Vector3f color = castRay(reflection_ray, depth + 1);
